...
enum sidlemode { IDLE_DETECTED, IDLE_REQUEST_NORMAL, SMART_IDLE };

enum Softreset { NORMAL_MODE, MODULE_RESET };

enum Autoidle { OCP_CLOCK_FREE, AUTOIDLE };

enum CLKG { CG_POW_2, CG_1 };

enum FFER { DEACTIVE_BUF, ACTIVE_BUF };

enum FFEW { T_BUF_NA, T_BUF_A };

enum TCS { CC_0_5, CC_1_5, CC_2_5, CC_3_5 };

enum SBPOL { POLARITY_LOW, POLARITY_HIGH };

enum SBE { DEFAULT_WL, START_SBPOL };

enum SPIENSLV { EN_1, EN_2, EN_3, EN_4 };

enum FORCE { SPILIN_DRIVE_LOW, SPILIN_DRIVE_HIGH };

enum TURBO { TUR_D, TUR_A };

enum INPUT_S { SPIDAT0, SPIDAT1 };

enum DPE1 { SPIDAT1_S, SPIDAT1_NS };

enum DPE0 { SPIDAT0_S, SPIDAT0_NS };

enum DMAR { R_DMA_DISABLED, R_DMA_ENABLED };

enum DMAW { RW_DMA_DISABLED, RW_DMA_ENABLED };

enum TRM { TR, RECEIVE_ONLY, TRANSMIT_ONLY };

enum EPOL { SPIEN_HIGH, SPIEN_LOW };

enum POL { SPICLK_HIGH, SPICLK_LOW };

enum PHA { DATA_LATCH_ODD, DATA_LATCH_EVEN };

enum FDAA { MCSPI_TX_RX_ENABLED, MCSPI_DAFTX_DAFRX };

enum MOA { MWL_ENABLED, MWL_FIFO };

enum INITDLY { NO_DELAY_SPI, SPI_BUS_4, SPI_BUS_8, SPI_BUS_16, SPI_BUS_32 };

enum SYSTEMT { FUNCTION_MODE, SYSTEST_MODE };

enum MS { MASTER_G_SPICLK, SLAVE_R_SPICLK };

enum PIN34 { SPIEN_CS, SPIEN };

enum SINGLE { MULTI_CH_MASTER, ONE_CH_MASTER };
...
enum RXFFE { RX_FIFO_BUF_NOT_FULL, RX_FIFO_BUF_FULL };

enum TXFFF { TX_FIFO_BUF_NOT_FULL, TX_FIFO_BUF_FULL };

enum TXFFE { TXE_FIFO_BUF_NOT_FULL, TXE_FIFO_BUF_FULL };

enum EOT { BEGIN_TANSFER, END_TANSFER };

enum TXS { REG_FULL, REG_EMPTY };
...
bool regaddr(mcspi_addr *addr, u32 base_addr) {
  ...
  addr->regaddr = NULL;
  addr->regaddr =
      pmap_mapdev((vm_paddr_t)base_addr, (vm_size_t)MCSPI_REGISTER_SIZE);
  if (!(addr->regaddr)) {
    addr->err = ENOMEM;
    return true;
  }
