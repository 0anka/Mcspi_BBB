...
#define DEFINE_CMR(__name, pullselect, pulltype, rxt)                          \
  struct conf_spi spi0_##__name = {                                            \
      .sctr = FAST, .rx = rxt, .ps = pullselect, .pt = pulltype}
...
enum Slewctrl { SLOW, FAST };

enum Rxactive { OUTPUT, BOTH };

enum Pullselect { SELECTDOWN, SELECTUP };

enum Pulltype { PULLDOWN, PULLUP };
